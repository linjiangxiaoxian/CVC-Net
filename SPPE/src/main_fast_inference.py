...
class InferenNet(nn.Module):
    def __init__(self, kernel_size, dataset):
        ...
        print('Loading pose model from {}'.format('./models/sppe/cvc_net.pth'))
        sys.stdout.flush()
        model.load_state_dict(torch.load('./models/sppe/cvc_net.pth'))
        model.eval()
        self.pyranet = model
    ...
    def forward(self, x):
        out = self.pyranet(x)
        out = out.narrow(1,16, 17)

        flip_out = self.pyranet(flip(x))
        flip_out = flip_out.narrow(1, 16, 17)
